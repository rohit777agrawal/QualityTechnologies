...
class LoginPage extends Component {
  ...
  handleSubmit(event) {
    ...
    if (!this.state.createAccount) {
      ...
      try {
        ...
        this.props.createNewLogin(this.state.userName, this.state.email, this.state.password).then((res) => (res.json())).then((json) => {
          if (!json.success) {
            throw json.error;
          }
        })
  ...
  render() {
    return (<> < Container fluid className = "vh-100 text-center col-3" > <Row className="h-100 justify-content-center align-items-center">
      <Col className="w-100">
        <h1>{
        ...
        <Form className="" target="">
          <Form.Group hidden={!this.state.createAccount} className="mb-3">
            ...
            <Form.Control placeholder="User Name" onChange={(e) => {
